...
public class LemonMetaObjectHandler implements MetaObjectHandler {
    ...
    public void insertFill(MetaObject metaObject) {
        ...
        Long userNo;
        userNo = Optional.ofNullable(SecurityUtil.getUser()).map(lemonUser -> Long.parseLong(lemonUser.getUsername())).orElse(0L);
    ...
    public void updateFill(MetaObject metaObject) {
        setFieldValByName(MODIFY_TIME, new Date(), metaObject);
        setFieldValByName(MODIFY_BY, Optional.ofNullable(SecurityUtil.getUser()).map(lemonUser -> Long.parseLong(lemonUser.getUsername())).orElse(0L), metaObject);
    }
}
